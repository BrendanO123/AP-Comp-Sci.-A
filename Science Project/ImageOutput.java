    ...
    public static BufferedImage WriteKey(BufferedImage img, String[] biomes, int[] colors, int allocatedHeight, int Gap_size, int Font_Size){
        ...
        /*for (int x=0;x<width;x++){
            for (int y=UpperBound; y<LowerBound; y++){
                if (x%maxSize==0){
                    if((y+totalSpacing)<LowerBound && y%totalSpacing==0){
                        if (count<biomes.length-1){
                            Writer.WriteKeyElement(colors[count], biomes[count], x, y);
                            count++;
                        }
                    }
                }
                if (img.getRGB(x, y)==(new Color(0, 0, 0).getRGB())){img.setRGB(x, y, Color.WHITE.getRGB());}
            }
        }*/
...
class BiomeImage{
    public static void GetBiomeImage(Biomes[][] BiomeMap){
        ...
        try{ImageIO.write(BiomeImage, "PNG", new File("BiomeImage.png"));}
