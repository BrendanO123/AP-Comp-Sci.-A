...
class Perlin2{
    ...
    public float[][] GetMap(int scale, byte interpolationDist){
        ...
        /*int count=0;
        /*for (int x=0; x<scale; x+=16){
            for (int y=0; y<scale; y+=16){
                Map[x][y]=0;
                count=0;
                for (byte i=0; i<8;i++){
...
class MapMixer{
    ...
    //float[][] ErosionMap, ContinentalnessMap, TempMap, HumidityMap, SlopeMap;
    ...
    public MapMixer(short[][] LandMap, Biomes[][] BiomeMap1, Random randNum, int LandMapMax, int LandMapMin){
        ...
        for (short x=0; x<2048;x++){
           for (short y=0; y<2048;y++){
                ...
                //if(value>0){value=(float)(value/(double)LandMapMax);}
                //else{value=(float)(value/(double)-LandMapMin);}
                //BiomeMapVariablesImage.setRGB(x+2048, y+2048, (value >= 0 ? new Color(0, (int)(value*255), 0).getRGB() : new Color(0, 0, (int)(255+value*255)).getRGB()));
            }
        }
        LandMap=null;
            ...
            for (short x=0; x<2048; x++){
                iteration : for (short y=0; y<2048; y++){
                        ...
                        //BiomeMapVariablesImage.setRGB(x+2048,y, new Color((int)((value+1)*127.5), 0, 0).getRGB());
            ...
            for (short x=0; x<2048; x++){
                iteration : for (short y=0; y<2048; y++){
                        ...
                        //BiomeMapVariablesImage.setRGB(x,y+2048, new Color(0, 0, (int)((value+1)*127.5)).getRGB());
        ...
        //try{ImageIO.write(BiomeMapVariablesImage, "PNG", new File("BiomeMapVariablesImage.PNG"));}
        ...
        */
        ...
        /*BufferedImage ErosionMapImage = new BufferedImage(2048, 2048, 1);
        FastNoiseLite Erosion = new FastNoiseLite();
        Erosion.SetSeed(seed);
        Erosion.SetFractalOctaves(4);
        Erosion.SetFractalGain(0.4f);
        Erosion.SetNoiseType(FastNoiseLite.NoiseType.ValueCubic);
        ...
        for (int x=0; x<2048; x++){
            for (int y=0; y<2048; y++){
                ErosionMap[x][y] = Erosion.GetNoise(x, y);
            }
        }
        
        ImageIO.write(ErosionMapImage, "PNG", new File("ErosionMapImage.png"));

        ErosionMapImage=null;*/

        /*short width = (short)LandMap.length;
        short length = (short)LandMap[0].length;

        BufferedImage BiomeImage = new BufferedImage(2048, 2048, 1);

        int shallowOceanThreshold=(int)-(FinishedStacks.max/4);
        int lowLandThreshold=(int)FinishedStacks.max/64;
        int inlandThreshold=(int)FinishedStacks.max/16;
        for (short x=0; x<width; x++){
            for (short y=0; y<length; y++){
                if (LandMap[x][y]>=0){
                    if (LandMap[x][y]>lowLandThreshold){
                        if (LandMap[x][y]>=inlandThreshold){
                            BiomeMap[x][y]=Biomes.Inland;
                            BiomeImage.setRGB(x,y, new Color(50,175,10).getRGB());
                        }
                        else{
                            BiomeMap[x][y]=Biomes.LowLand;
                            BiomeImage.setRGB(x,y, new Color(100,200,0).getRGB());
                        }
                    }
                    else{
                        BiomeMap[x][y]=Biomes.Shore;
                ...
                else if (LandMap[x][y]>=shallowOceanThreshold){
                    BiomeMap[x][y]=Biomes.ShallowOcean;
        ...
        ImageIO.write(BiomeImage, "PNG", new File("BiomeImage.png"));
